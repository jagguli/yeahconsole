...
char *opt_term;
KeyShortCut opt_key;
KeyShortCut opt_key_smaller;
KeyShortCut opt_key_bigger;
KeyShortCut opt_key_full;
Cursor cursor;
int resize_inc;
void roll(int i);
void get_defaults(void);
KeyShortCut grab_that_key(char *opt, unsigned int numlockmask);
void init_win(void);
    ...
    while (1) {
        ...
        switch (event.type) {
            ...
            case KeyPress:
                ...
                if (key == opt_key.keysym && event.xkey.state == opt_key.modmask) {
                    if (!hidden) {
                ...
                if (!hidden) {
                    if (key == opt_key_full.keysym && event.xkey.state ==opt_key_full.modmask){
                        if (!fullscreen) {
                            old_height = height;
                    ...
                    if (key == opt_key_bigger.keysym)
                        height += resize_inc;
                    if (key == opt_key_smaller.keysym)
                        height -= resize_inc;
